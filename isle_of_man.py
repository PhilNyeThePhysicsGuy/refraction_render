...
raster = Image.open("n54_w005_1arc_v3.tif")
...
#   cfunc=cfunc,cfunc_args=(621,50000),disp=False)




renderer = Renderer_35mm(calc,h_obs,lat_obs,lon_obs,(lat_lighthouse,lon_lighthouse),
	d_max,vert_res=6000,focal_length=2000)

pr = cProfile.Profile()
pr.enable()
renderer.render_scene(s,'lighthouse_render.png',cfunc=cfunc,cfunc_args=(621,50000),disp=False)
pr.disable()
...
pr = cProfile.Profile()
pr.enable()
renderer.render_scene(s,'lighthouse_render.png',cfunc=cfunc,cfunc_args=(621,50000),disp=False,gpu=True)
pr.disable()
pr.print_stats(sort='time')
